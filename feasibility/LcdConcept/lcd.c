...
void Crystalfontz128x128_SetDrawFrame(uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1)
{
    ...
    HAL_LCD_writeData(x0 >> 8); // [15:8]
    HAL_LCD_writeData(x0); // [7:0]
    HAL_LCD_writeData(x1 >> 8); // [15:8]
    HAL_LCD_writeData(x1); // [7:0]
    ...
    HAL_LCD_writeData(y0 >> 8); // [15:8]
    HAL_LCD_writeData(y0); // [7:0]
    HAL_LCD_writeData(y1 >> 8); // [15:8]
    HAL_LCD_writeData(y1); // [7:0]
}
